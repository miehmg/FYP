...
struct Solution
{
    ...
    Solution(StaticGraph &sg) : static_graph(sg) { vertice_states.reserve(sg.order_size); }

    Solution(StaticGraph &sg, vector<Route> &vsg) : static_graph(sg), vehicle_routes(vsg) { vertice_states.reserve(sg.order_size); }
};
...
Solution NearestInsertionSolve(StaticGraph &);

Solution StaticAntSolve(StaticGraph &);

// Solution StaticTabuSolve(StaticGraph &);

Solution DynamicTabuSolve(StaticGraph &, int wait_time);
