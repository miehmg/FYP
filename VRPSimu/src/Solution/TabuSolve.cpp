...
// Solution StaticTabuSolve(StaticGraph &static_graph)
// {
// }

Solution DynamicTabuSolve(StaticGraph &static_graph, int wait_tm)
{
    int wait_seed = 0;
    Solution solut(static_graph);
    // vector<PDVertice> &vertices = static_graph.vertices;
    // vector<Vehicle> &vhcl_stats = solut.vehicle_states;
    vector<Route> &vhcl_routes = solut.vehicle_routes;
    vector<VerticeState> &vrtc_stats = solut.vertice_states;

    int tm_hrz = INT32_MIN;
    bool cndt = true;
    bool ft = true;

    //initialize solution order state
    for (int i = 0; i != static_graph.vertices.size(); ++i)
    {
        vrtc_stats.emplace_back(static_graph.vertices[i]);
    }
    ...
    while (tm_hrz != INT32_MAX)
    {
        ...
        for (int i = 0; i != avlb_pdv.size(); ++i)
        {
            ...
            // {
            //     Route r{vhclsize};
            //     r.move_chain.emplace_back(static_graph.vehicle_depots[vhclsize]);
        ...
        for (int i = 0; i != avlb_pdv.size(); ++i)
        {
            ...
            for (int j = 0; j != avlb_vhcl.at(vrtc_num).size(); ++j)
            {
                ...
                if (tmp_vhcl_routes.find(vj) == tmp_vhcl_routes.end())
                {
                    ...
                    {
                        shared_ptr<VrtuVertice> vp = std::make_shared<VrtuVertice>(static_graph.vehicle_depots[next_vhcl_number]);
            ...
            else
            {
                if (tmp_vhcl_routes.find(vhcl_num) == tmp_vhcl_routes.end())
                {
                    ...
                    {
                        lstdest.vertice = std::make_shared<VrtuVertice>(static_graph.vehicle_depots[vhcl_num]);
                        lstdest.compl_time = tm_hrz;
                    }
        ...
        if (nextvrtc != INT32_MIN)
        {
            ...
            for (int i = vhcl_routes.size(); i <= nextvhcl; ++i)
            {
                ...
                VrtuVertice v = static_graph.vehicle_depots[i];
