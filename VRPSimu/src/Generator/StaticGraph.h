...
struct StaticGraph
{
    ...
    StaticGraph(int gs, int os, int ts, int tt, int spd, int vs = 0)
        : geo_size(gs), order_size(os), time_span(ts), tolerate_time(tt), speed(spd), vehicle_size(vs) { vehicle_size = vs > 0 ? vs : os; }

    friend void InitializePDVertices(StaticGraph &sg, const vector<int> &tm_seq, const vector<GraphCoor> &vhc_strt_pos, const int seed);
};
...
void InitializePDVertices(StaticGraph &, const vector<int> &, const vector<GraphCoor> &vhc_strt_pos, const int);
